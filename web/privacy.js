...
export default {
  /**
   * Webhook handler for new orders - automatically sends to external database API
   */
  ORDERS_CREATE: {
    ...
    callback: async (topic, shop, body, webhookId) => {
      ...
      try {
        ...
        // This is where we send the order data TO your external system (e.g., database, ERP)
  ...
  ORDERS_UPDATED: {
    ...
    callback: async (topic, shop, body, webhookId) => {
      try {
        ...
        console.log(
          "📤 Sending updated order to external API:",
          externalEndpoint
        );
        ...
        if (!response.ok) {
          ...
          console.log(
            "✅ Order update webhook sent successfully:",
            orderData.name
          );
        }
      } catch (error) {
  ...
  CUSTOMERS_DATA_REQUEST: {
    ...
    callback: async (topic, shop, body, webhookId) => {
      try {
        ...
        console.log(
          `Customer ${customer.id} (${customer.email}) requested data export`
        );
  ...
  CUSTOMERS_REDACT: {
    ...
    callback: async (topic, shop, body, webhookId) => {
      try {
        ...
        console.log(
          `Customer ${customer.id} (${customer.email}) data redaction requested`
        );
  ...
  SHOP_REDACT: {
    ...
    callback: async (topic, shop, body, webhookId) => {
      try {
        ...
        console.log(
          `Shop ${shop_domain} (ID: ${shop_id}) data redaction requested`
        );
